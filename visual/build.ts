...
let conteudoDoModal = document.getElementById('modal-content') as HTMLElement
let precoDoCarrinho = document.getElementById('precoCarrinho') as any

function append(parent: HTMLElement, el: HTMLElement) {
    return parent.appendChild(el);
}

function adicionarNoCarrinhoArray(div:any){
    let titulo:any,conteudoDoCatalogo
    titulo = div.firstChild.firstChild.innerHTML
    conteudoDoCatalogo = catalogo.itensNoCarrinho
    conteudoDoCatalogo.forEach((item:any)=>{
        if(titulo === item.nome){
            carrinho.newItem(item.nome,item.vaiBem,item.slogan,
                item.ph,item.preco,item.estoque)
        }
    })
}
...
function adicionar(btn : HTMLButtonElement){
    btn.onclick = function(){
        criarItem(btn)
    }
}

function criarItem(btn : HTMLButtonElement){
    let controle = 0;
    ...
    if(qtdElementosModal > 0){
        ...
        arrItens.forEach((item:any) => {
            let title = item.getElementsByClassName('title');
            let secondTitle = elementoDoCatalogo?.getElementsByTagName('h1') as any;
...
function btnsItens(qtd:any,add:HTMLButtonElement,
    sub:HTMLButtonElement,div:any){
    ...
    sub.onclick = function() {
        ...
        if(clicks <= 0){
            ...
            let titulo:any,conteudoDoCarrinho:any
            titulo = div.firstChild.firstChild.innerHTML
            conteudoDoCarrinho = carrinho.itensNoCarrinho
            conteudoDoCarrinho.forEach((item:any,i:number)=>{
                if(titulo === item.nome){
